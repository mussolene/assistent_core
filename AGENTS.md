...
3. **Сообщение коммита** — точно описывает, что сделано (на русском или английском, в одном стиле). Для автоматического changelog в релизах использовать [Conventional Commits](https://www.conventionalcommits.org/): см. **docs/CONVENTIONAL_COMMITS.md**.
4. **Запись о сделанном** — после каждой итерации (фича, блок дорожной карты, доработка) обязательно зафиксировать результат, чтобы состояние не потерялось: обновить **docs/ITERATIONS_ROADMAP.md** (новый блок или галочка [x], секция «Прочие доработки» при необходимости) и/или связанные docs; затем **коммит** с этими изменениями. Итог итерации = код + запись в roadmap/docs + коммит.

Цикл итерации: **Аналитика → Разработка → Тестирование → Исправление → Приёмка → Аудит**. После приёмки — регрессия по проекту, **запись в roadmap/docs**, затем **коммит**.
