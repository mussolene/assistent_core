...
1. **Коммиты обязательны** — по завершении логического блока изменений выполнять `git add` (затронутые файлы) и `git commit`. Не оставлять репозиторий без коммита после выполненной работы.
2. **Push не делать** — в удалённый репозиторий не пушить по умолчанию. Push только по явной просьбе пользователя. Это требование к разработке текущего репозитория.
