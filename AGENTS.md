...
1. **Коммит без push** — выполнить `git add` (затронутые файлы) и `git commit`.
2. **Сообщение коммита** — точно описывает, что сделано в этой итерации (на русском или английском, в одном стиле).
3. **Не пушить** в удалённый репозиторий по умолчанию (push только по явной просьбе пользователя).
