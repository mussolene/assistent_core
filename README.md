...
[![Version](https://img.shields.io/badge/version-0.2.2-informational)](https://github.com/mussolene/assistent_core)
...
- **MCP-сервер для агента** — проект можно подключить в Cursor как MCP: уведомления в Telegram, запрос confirm/reject, обратная связь через `/dev` ([docs/MCP_DEV_SERVER.md](docs/MCP_DEV_SERVER.md)). Чтобы уведомления (notify) доходили, задайте **Chat ID для MCP** в дашборде: Каналы → Telegram → «Chat ID для MCP/агента», либо добавьте пользователя в разрешённые — тогда используется первый ID из списка.

### Уведомления MCP (почему не доходят и как исправить)

Цепочка: MCP/код вызывает `notify` → публикация в Redis (`assistant:outgoing_reply`) → **telegram-adapter** подписан на этот канал и шлёт сообщение в Telegram. Если уведомления не приходят:

1. **Не задан Chat ID** — в дашборде: **Каналы → Telegram** укажите «Chat ID для MCP/агента» (для личного чата = ваш User ID) или добавьте хотя бы одного пользователя в разрешённые — тогда используется первый ID из списка.
2. **Redis недоступен из процесса MCP** — MCP-сервер (дашборд или отдельный процесс) и telegram-adapter должны использовать один и тот же Redis (`REDIS_URL`). Проверьте переменную окружения для процесса, из которого вызывается notify.
3. **telegram-adapter не запущен или не подписан** — при старте адаптера в логах должно быть: `Subscribed to assistant:outgoing_reply and stream for Telegram`. Если Chat ID не задан, в логах будет предупреждение; при первом вызове `notify` без Chat ID в логах MCP появится предупреждение «Задайте TELEGRAM_DEV_CHAT_ID…».
...
3. **Привязка пользователей** — два способа (можно комбинировать):
   - **Одобрение заявок:** пользователь нажимает Start в боте (/start) → в дашборде в блоке «Ожидают одобрения» появляется заявка с именем и username → нажмите «Одобрить» или «Отклонить».
