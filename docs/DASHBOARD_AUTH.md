...
```mermaid
flowchart LR
    A[Запрос к маршруту] --> B{Есть cookie assistant_sid?}
    B -->|Нет| C{Есть пользователи в Redis?}
    B -->|Да| D[Загрузить сессию из Redis]
    C -->|Нет| E["/setup → создать owner"]
    C -->|Да| F["/login"]
    D --> G{Сессия валидна?}
    G -->|Да| H[require_role? → доступ по роли]
    G -->|Нет| F
    E --> I[Cookie + редирект на панель]
    F --> J[Логин/пароль → verify_user]
    J --> K{OK?}
    K -->|Да| I
    K -->|Нет| F
    H --> L[Рендер страницы]
