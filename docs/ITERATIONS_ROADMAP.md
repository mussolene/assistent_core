...
| Итерация | Статус | Содержание | Критерии приёмки |
|----------|--------|------------|-------------------|
| **9.1** | [ ] | **Markdown в Telegram:** сообщения приходят со «сырыми» знаками разметки. У Telegram свой формат (Markdown/MarkdownV2/HTML). Адаптировать исходящие сообщения: конвертация/экранирование под выбранный parse_mode, чтобы в чате отображалась корректная разметка (жирный, код, списки). | Все исходящие сообщения в Telegram отображаются без лишних символов; тесты. |
...
| **9.3** | [ ] | **Favicon дашборда:** сгенерировать иконку сайта, разместить в статике дашборда, подключить в разметке всех страниц. Сейчас вкладка показывает пустой лист. | В корне дашборда отдаётся favicon; иконка отображается во вкладке браузера. |
...
**Сделано:** 1.1, 1.2, 1.3, 2.1, 2.2, 5.1, 5.2, 6.1.  
**Дальше (на выбор):** 3.1 (документы в боте), 4.1 (Email в дашборде), 6.2 (страница репо), 7.1 (index_repo), 8.1 (память разговоров), **9.1** (Markdown в Telegram), **9.2** (команды репо в боте), **9.3** (favicon дашборда).
