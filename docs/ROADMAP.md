...
**Аналитика и фокус продукта (2026):** см. [docs/ANALYTICS_AND_ROADMAP_2026.md](ANALYTICS_AND_ROADMAP_2026.md) — оценка текущего состояния, тренды, минимизация зависимостей, память/контекст независимо от модели, полуформализация задач, интеграции To-Do и Google Calendar, MCP как исполнитель, приоритетная дорожная карта на 2026 год.
...
| **§3.3 Команды бота** | ✅ Готово | /status, /restart (TELEGRAM_ADMIN_IDS), флаг в Redis. |
| **§3.4 Разграничение по ролям** | ✅ Готово | TELEGRAM_ADMIN_IDS для /restart; в /help блок «Для админов» с /restart (BOT_COMMANDS.admin_only). |
| **§3.5 Первичная инициализация** | ✅ Реализовано | /setup, создание первого owner, wizard — в auth.py и app.py (setup_done, create_user, create_session). |
| **Этап A (авторизация)** | ✅ Реализовано | Модель пользователей и сессий в Redis (auth.py), /login, /logout, /api/session, require_auth, роли owner/operator/viewer. |
| **Этап B (первичная настройка)** | ✅ Реализовано | Флаг setup_done, страница /setup, создание первого owner, редирект на главную. |
| **Этап D (мониторинг и динамика)** | ✅ Готово | Мониторинг расширен, формы через fetch. |
| **Этап E (команды бота и роли)** | ✅ Готово | /status, /restart, TELEGRAM_ADMIN_IDS. |
| **Этап F (перезапуск кластера)** | ✅ Готово | scripts/restart_worker.py, docs/RESTART_WORKER.md. |

**ITERATIONS_ROADMAP.md:** все блоки 1–10 отмечены как выполненные.  
**UX_UI_ROADMAP.md:** фазы 1–5 и п.4.3 (аккордеон) выполнены.
...
**Реализовано (3.2):** общий JS `static/js/app.js` (showToast, apiPostForm); формы **Telegram**, **Модель**, **Email** и **Данные** отправляются через fetch при наличии app.js, ответ JSON (success/error), toast при успехе/ошибке, кнопка «Сохранить» блокируется на время запроса. POST /save-telegram, /save-model, /save-email, /save-data при заголовках Accept: application/json или X-Requested-With: XMLHttpRequest возвращают JSON вместо редиректа.
...
**Этап F реализован:** скрипт `scripts/restart_worker.py` опрашивает ключ `assistant:action:restart_requested`, при наличии выполняет команду из `RESTART_CMD` и удаляет ключ. См. **docs/RESTART_WORKER.md**.
