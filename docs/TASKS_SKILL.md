...
- **set_reminder(task_id, reminder_at)** — обязательны оба параметра; `reminder_at` в формате ISO datetime (например `2025-02-22T12:35:00+00:00`). Для «напомни про неё через 5 минут» ассистент сначала определяет задачу (из контекста или list_tasks), затем вычисляет reminder_at = сейчас + 5 минут и вызывает set_reminder.
...
- **Список в Telegram**: только актуальные задачи (open, end_date ≥ сегодня); кнопка «✓ Выполнена» (callback `task:done:{id}`); сообщение со списком удаляемое/заменяемое.
- **Компактный список** неактуальных; **архивация** выполненных; **архив** с просмотром за период и **поиск по смыслу** с датами.
